package com.spark.devproj.sparkSQL.dataAnalysis
...
object StackOverFlowSurvey {
  def main(args: Array[String]): Unit = {
    ...
    val sparkSession = SparkSession
      .builder()
      .config(SparkConfigs.getLocalSparkConf("StackOverFlowSurvey"))
      .getOrCreate()
    ...
    val responses = dataFrameReader
      ...
      .csv(CommonUtils.getInputFilePath("2016-stack-overflow-survey-responses.csv"))
    ...
    responseWithSelectedColumns
      .filter(responseWithSelectedColumns.col("country").===("Afghanistan")).show()

    System.out.println("=== Print the count of occupations , country ===")
    responseWithSelectedColumns.groupBy("occupation", "country").count().show()
